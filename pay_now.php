...
if (isset($_POST['pay_now'])) {
    ...
    $query1 = "INSERT INTO `booking_order`(`user_id`, `car_id`, `booking_date`, `dropout_date`, `order_id`, `trans_amt`, `trans_resp_msg`, `trans_status`) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    insert($query1, [
        $_SESSION['uId'],
        $_SESSION['car']['id'],
        ...
        $trans_status // Set the transaction status here
    ], "isssssds");
    ...
    // Update the trans_id in the booking_order table
    $trans_id = $final['data']['instrumentResponse']['merchantTransactionId'];
    $query4 = "UPDATE `booking_order` SET `trans_id` = ?, `trans_status` = ?, `trans_resp_msg` = ? WHERE `order_id` = ?";
    update($query4, [$trans_id, 'Pending', 'Transaction initiated', $ORDER_ID], "ssss");

    // Insert the payment details into the database
    $trans_status = ($final['data']['instrumentResponse']['status'] === 'SUCCESS') ? 'PAYMENT_SUCCESS' : 'PAYMENT_FAILED';
    $trans_resp_msg = ($final['data']['instrumentResponse']['status'] === 'SUCCESS') ? 'Payment successfully completed' : 'Payment failed';
